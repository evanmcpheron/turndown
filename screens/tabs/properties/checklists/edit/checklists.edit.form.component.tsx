...
export const ChecklistsEditForm = forwardRef<
  {
    submitData: (
...
>(({ selectedId }, ref) => {
  ...
  const saveData = async (data: TurndownObject) => {
    try {
      ...
      await checklistApi.update({ ...cleaned, is_: true }, selectedId);
  ...
  return (
    <View style={{ gap: 10, height: "100%" }}>
      <TurndownSection
        title="Checklist"
        hint="What is the name of this checklist ?"
      >
      ...
      {isModalDisplayed && (
        <Modal
          ...
          header={{
            primary: "Add Item",
            secondary: "Manage items that belong to a checklist ",
          }}
          saveText="Add Item"
          onCancel={() => {
