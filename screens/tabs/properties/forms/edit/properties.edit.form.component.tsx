...
export const PropertiesEditForm = forwardRef<
  ...
  PropertiesEditFormProps
>(({ propertyId }, ref) => {
  const { app } = useTheme();
  const s = useMemo(() => propertiesEditFormStyles(app), [app]);
  ...
  useEffect(() => {
    propertiesApi.getById(propertyId).then((response) => {
  ...
  const saveData = async (data: TurndownObject) => {
    try {
      ...
      await propertiesApi.updateById(cleaned, propertyId);
      return true;
    } catch {
